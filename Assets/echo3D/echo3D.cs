...
public class echo3D : MonoBehaviour
{
    ...
    private string secKey = "<YOUR_SECRET_KEY>";
    ...
    void Start()
    {
        ...
        serverURL = endpointURL + "/query?key=" + APIKey + "&secKey=" + secKey + "&src=" + src;
        ...
        try
        {
            ...
            // Want to query a single entry? Replace the above line with:
    ...
    public IEnumerator UploadModel(byte[] objFile, string objFileName, byte[] mtlFile, string mtlFileName, List<byte[]> pngFiles, List<string> pngFileNames)
    {
        ...
        form.AddField("src",  src);         // Query source
        ...
        // Send request
        UnityWebRequest www = UnityWebRequest.Post(endpointURL + "/upload", form);
        yield return www.SendWebRequest();
        // Check for errors
        if (www.isNetworkError || www.isHttpError)
        {
    ...
    public IEnumerator UpdateEntryData(string entryID, string dataKey, string dataValue)
    {
        ...
        UnityWebRequest www = UnityWebRequest.Post(endpointURL + "/post", form);
        // Send request
        yield return www.SendWebRequest();
        // Check for errors
        if (www.isNetworkError || www.isHttpError)
